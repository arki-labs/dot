...
function renderTextManifest(manifest) {
    ...
    if (manifest.actions.length === 0) {
        ...
        for (const action of manifest.actions) {
            lines.push(`  ${pad(action.id, widthId)}  ${pad(action.binding, widthBinding)}  ${pad(action.direction, 3)}  ${pad(action.address ?? '-', widthAddress)}  ${action.plugin}`);
        }
    }
    lines.push('');
    // Projections
    ...
    if (manifest.projections.length === 0) {
        ...
        for (const projection of manifest.projections) {
            lines.push(`  ${pad(projection.format, widthFormat)}  ${pad(projection.binding, widthBinding)}  ${pad(projection.module, widthModule)}  ${projection.plugin}`);
        }
    }
    lines.push('');
    // Dependencies
