    ...
    async #runDisposeForRecords(records, phaseLogger) {
        ...
        for (const record of records) {
            for (const value of Object.values(record.publishedServices)) {
                if (isLazy(value))
                    ownerOf.set(value, record);
            }
        }
        for (const record of records) {
            const seen = new Set();
            const lazyPublishes = Object.entries(record.publishedServices).filter((entry) => {
                const value = entry[1];
                if (!isLazy(value) || ownerOf.get(value) !== record || seen.has(value))
                    return false;
                seen.add(value);
                return true;
            });
